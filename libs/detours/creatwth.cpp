                                        ...
                                        _In_ PDETOUR_CREATE_PROCESS_ROUTINEA pfCreateProcessA)
{
    ...
    if (DWORD nLen = GetEnvironmentVariableA("WINDIR", szExe, ARRAYSIZE(szExe)); nLen == 0 || nLen >= ARRAYSIZE(szExe)) {
        goto Cleanup;
    }
                                        ...
                                        _In_ PDETOUR_CREATE_PROCESS_ROUTINEW pfCreateProcessW)
{
    ...
    if (!AllocExeHelper(&helper, dwTargetPid, nDlls, rlpDlls)) {
		...
		if (DWORD nLen = GetEnvironmentVariableW(L"WINDIR", szExe, ARRAYSIZE(szExe));nLen == 0 || nLen >= ARRAYSIZE(szExe)) {
        goto Cleanup;
    }
